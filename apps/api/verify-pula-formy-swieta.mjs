...
await j(await aAdmin('/pools/default', { method: 'PUT', body: JSON.stringify({ value: 15, employmentType: 'OUT' }) }));
ok((await bal(uop)) === 26, 'UoP dziedziczy pulę wspólną (26)');
ok((await bal(b2b)) === 20, 'B2B ma własną pulę (20)');
ok((await bal(out)) === 15, 'OUT ma własną pulę (15)');
...
ok(poolOf(uop.id) === 26 && poolOf(out.id) === 15, 'raport wykorzystania liczy pulę per forma (26 / 15)');
